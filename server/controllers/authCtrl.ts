...
import preferanceModel from "../models/preferanceModel";
...
const authCtrl = {
  ...
  refreshToken: async (req: Request, res: Response) => {
    try {
      ...
      const preferance = await preferanceModel
      ...
      if (preferance) {
        res.json({
          msg: "Login Success!",
          access_token,
          user: { ...user._doc, ...preferance._doc, password: "" },
        });
      } else
        res.json({
          msg: "Login Success!",
  ...
  googleLogin: async (req: Request, res: Response) => {
    try {
      ...
      const password = email + "zshgvfjhgfjsdhc fjfd  fgjk";
      const passwordHash = await bcrypt.hash(password, 12);
